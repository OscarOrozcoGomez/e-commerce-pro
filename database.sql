...
CREATE TABLE IF NOT EXISTS `purchase_order_postponed_items` (
  ...
  INDEX `idx_po_postergado_estado` (`estado`),
  INDEX `idx_po_postergado_almacen` (`id_almacen`),
...
CREATE TABLE IF NOT EXISTS `pedidos` (
  ...
  `estado` ENUM('pendiente_pago','pagado','en_reparto','entregado','cancelado') NOT NULL DEFAULT 'pendiente_pago',
  ...
  `prioridad_entrega` TINYINT UNSIGNED NOT NULL DEFAULT 0,
  `tiempo_servicio_min` SMALLINT UNSIGNED NOT NULL DEFAULT 5,
  `fecha_entrega` DATETIME DEFAULT NULL,
  PRIMARY KEY (`id_pedido`),
  UNIQUE KEY `uq_pedidos_numero` (`numero_pedido`),
  INDEX `idx_pedidos_almacen` (`id_almacen`),
  INDEX `idx_pedidos_repartidor` (`id_repartidor`),
  ...
  CONSTRAINT `fk_pedidos_repartidor` FOREIGN KEY (`id_repartidor`) REFERENCES `usuarios` (`id_usuario`) ON DELETE SET NULL ON UPDATE CASCADE,
