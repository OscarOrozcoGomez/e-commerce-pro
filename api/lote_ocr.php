...
try {
    ...
    if ($rutaLote === null && $rutaTabla === null && $codigoBarras === '') {
        echo json_encode(['success' => false, 'message' => 'No se recibió ninguna imagen ni código de barras.']);
        exit;
    }
    ...
    // Match por código de barras (funciona aunque el OCR esté apagado).
    $productoMatch = null;
    if ($codigoBarras !== '') {
        $productoMatch = loteBuscarProductoPorCodigoBarras(getPDO(), $codigoBarras);
    }

    if ($rutaLote === null && $rutaTabla === null) {
        echo json_encode([
            'success' => true,
            'data' => ['codigo_barras' => $codigoBarras, 'producto_match' => $productoMatch, 'fuente' => 'barcode'],
    ...
    if (!loteOcrHabilitado()) {
        echo json_encode([
            ...
            'foto' => $fotoRelativa,
            'data' => ['codigo_barras' => $codigoBarras ?: null, 'producto_match' => $productoMatch],
        ]);
        exit;
    }
    ...
    if ($codigoBarras !== '') {
        $rutas['codigo_barras'] = $codigoBarras;
    }

    $datos = loteOcrProcesar($rutas);
    $datos['foto'] = $fotoRelativa;
    $datos['producto_match'] = $productoMatch;
