...
try {
    ...
    if ($rol === 'admin') {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as total FROM pedidos WHERE id_repartidor IS NOT NULL AND estado = 'pagado' AND id_almacen = ?");
        $stmt->execute([$idAlmacen]);
        $stats['por_entregar'] = $stmt->fetch();

    } elseif ($rol === 'repartidor') {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM pedidos WHERE id_repartidor = ? AND estado = 'pagado'");
