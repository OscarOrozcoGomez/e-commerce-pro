...
/**
 * Extrae capsulas por envase / porcion de un texto de tabla nutrimental.
 *
 * @return array{capsulas_por_envase: ?int, porcion_capsulas: ?int}
 */
function loteOcrExtraerCapsulas(string $texto): array
{
    ...
    if (preg_match('/(\d{2,4})\s*(?:c[aá]psulas?|c[aá]ps\.?|tabletas?|softgels?|comprimidos?)\b/u', $t, $m)) {
        $porEnvase = (int) $m[1];
    }
    if (preg_match('/(?:contenido neto|net\s*wt|contiene)\D{0,15}(\d{2,4})\s*(?:c[aá]psulas?|c[aá]ps\.?)/u', $t, $m)) {
        $porEnvase = (int) $m[1];
    }
    if (preg_match('/(?:porci[oó]n|tama[nñ]o de la porci[oó]n|serving size)\D{0,15}(\d{1,2})\s*(?:c[aá]psulas?|c[aá]ps\.?)/u', $t, $m)) {
        $porcion = (int) $m[1];
    }
    if ($porcion === null && preg_match('/(?:tomar|ingerir|consumir)\D{0,10}(\d{1,2})\s*(?:c[aá]psulas?|c[aá]ps\.?)/u', $t, $m)) {
        $porcion = (int) $m[1];
    }

    return ['capsulas_por_envase' => $porEnvase, 'porcion_capsulas' => $porcion];
}
...
function loteOcrInterpretar(string $textoLote, ?string $textoTabla = null, ?callable $llm = null): array
{
    ...
    $capsDet = loteOcrExtraerCapsulas(($textoTabla ?? '') . "\n" . $textoLote);
    ...
    if (!empty($json['fecha_caducidad']) && is_string($json['fecha_caducidad'])) {
        $ts = strtotime($json['fecha_caducidad']);
        if ($ts !== false) {
            $out['fecha_caducidad'] = date('Y-m-d', $ts);
            $out['caducidad_aproximada'] = !empty($json['caducidad_aproximada']);
        }
    }
    foreach (['capsulas_por_envase', 'porcion_capsulas'] as $k) {
