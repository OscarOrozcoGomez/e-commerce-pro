...
$pdo = getPDO();
$sql = "SELECT p.*,         COALESCE(NULLIF(TRIM(p.imagen), ''), NULLIF(TRIM(p.imagen_url), ''), NULLIF((SELECT pi.ruta_archivo FROM producto_imagenes pi INNER JOIN productos p_img ON pi.id_producto = p_img.id_producto WHERE (p_img.id_producto = p.id_producto OR p_img.id_padre = p.id_producto OR (TRIM(p_img.nombre) = TRIM(p.nombre) AND p_img.estado = 'activo')) ORDER BY (p_img.id_producto = p.id_producto) DESC, (p_img.id_padre = p.id_producto) DESC, pi.orden ASC LIMIT 1), '')) as imagen,        (SELECT MIN(precio_venta) FROM productos p3 WHERE (p3.id_producto = p.id_producto OR p3.id_padre = p.id_producto) AND p3.estado = 'activo') as precio_desde,
